            ...
            case ID_BTN_CONVERT:
                {
                    ...
                    bool decToBinMode = (SendMessage(GetDlgItem(hwnd, ID_RAD_DEC_BIN), BM_GETCHECK, 0, 0) == BST_CHECKED);

                    std::string resultStr;
                    if (decToBinMode)
                    {
                        if (IsDecimal(inputStr.c_str()))
                        {
                            resultStr = ConvertDecToBin(inputStr);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            resultStr = ConvertBinToDec(inputStr);
                        }
                        else
                        {
                            resultStr = "Bao loi du lieu khong hop le";
